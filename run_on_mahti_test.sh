...
PYTHONWARNINGS="ignore::Warning" timeout 4m srun python main_train.py --common.config-file config/classification/convnext.yaml --common.results-loc results/convnext --dataset.root-train "$LOCAL_SCRATCH/train" --dataset.root-val "$LOCAL_SCRATCH/val" --dataset.workers 10
if [[ $? -eq 124 ]]; then
